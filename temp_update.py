...
new_style = '''
    ...
    /* Override old section styling to make it cleaner */
    .form-section {
        border: none;
        padding-bottom: 0;
        margin-bottom: 0;
...
# Find the end of styles and inject
if '.alert-error {' in content:
    content = content.replace('.alert-error {', new_style + '\\n    .alert-error {')

# Modify the form structure
form_start = '''    <form id="profileCreationForm" method="post" enctype="multipart/form-data">
        {% csrf_token %}'''
...
content = content.replace('<!-- 2. EXPERTISE & LOGISTIQUE -->', '''            <div class="form-actions">
                <div></div>
...
content = content.replace('<!-- 3. GESTION DES DIPLOMES -->', '''            <div class="form-actions">
...
# Modify Step 3 end and submit button
submit_section = '''            <button type="submit" class="submit-btn" id="mainSubmitBtn" style="margin-top: 2rem;">
...
new_submit_section = '''            <div class="form-actions">
...
content = content.replace(submit_section, new_submit_section)


# Add JS validation and stepping logic at the end of the script block
js_logic = '''
    // Stepper Logic
    let currentStep = 1;
    const totalSteps = 3;
    ...
    function validateStep(step) {
        ...
        if(!isValid) {
            // Shake effect or simple alert for ux
