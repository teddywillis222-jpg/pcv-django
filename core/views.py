...
def admin_api_ambassadeurs(request):
    ...
    try:
        ...
        return HttpResponse("<div>L'application referrals n'est pas install�e.</div>")
...
def admin_api_ambassadeurs_action(request):
    "Action sur les ambassadeurs (payer une r�compense, annuler, etc.)."
    ...
    try:
        ...
        if action == 'mark_paid':
            if referral.status == 'REWARD_PENDING' and hasattr(referral, 'reward'):
                ...
                return JsonResponse({'error': '�tat invalide pour paiement.'}, status=400)
